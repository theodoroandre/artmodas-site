...
export default function ProdModal({ prod, onClose, onSave }) {
  const [f, setF] = useState(prod || { nome: "", cat: "", custo: "", preco: "", estoque: "0", minimo: "2", cod: "" });
    ...
    <div className="ov" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="mod">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 13 }}>
          <div>
            <label className="lbl">Codigo de Barras</label>
            <div style={{ display: "flex", gap: 8 }}>
              <input className="inp" value={f.cod || ""} onChange={s("cod")} placeholder="Ex: 7891234567890" style={{ flex: 1 }} />
              <button className="btn ghost" type="button" onClick={() => setScanning(true)} style={{ padding: "8px 12px", whiteSpace: "nowrap", fontSize: 16 }}>📷</button>
            </div>
          </div>
      ...
      {scanning && <BarcodeScanner onScan={(code) => { setF((p) => ({ ...p, cod: code })); setScanning(false); }} onClose={() => setScanning(false)} />}
    </div>
  );
}
